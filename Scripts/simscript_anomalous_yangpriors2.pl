...
foreach my $ngenes(50, 100, 150, 200, 300, 400) { 
   ...
   while ( $count < NLOOP1)
   {
       ...
       #### Yang priors 2 ####
       system(qq( sed 's/nloci = XX/nloci = $ngenes/g' a01_yang2.ctl > a01.ctl));
   ...
   system("sed -f sed.tree.ind alldat.out > alldat_a1_005_200bp_yangpriors2_$ngenes.out");
...
foreach my $ngenes(50, 100, 150, 200, 300, 400) { 
   ...
   system("paste bpp_reps.out bppd_reps.out svdq.out valid.svdq > alldat.out");
   system("sed -f sed.tree.ind alldat.out > alldat_a1_005_1000bp_yangpriors2_$ngenes.out");
